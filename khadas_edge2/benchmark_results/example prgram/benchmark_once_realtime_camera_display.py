...
# Siapkan file hasil log
logfile = os.path.join(output_dir, f"benchmark_power_realtime_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
for model_path, imgsz in models:
    ...
    print(f"\n⏳ Persiapan evaluasi: {model_name} | Resolusi: {imgsz}x{imgsz}")

    # Tampilkan info countdown selama 10 detik
    info_frame = 255 * np.ones((240, 640, 3), dtype=np.uint8)
    for sec in range(10, 0, -1):
        frame_copy = info_frame.copy()
        cv2.putText(frame_copy, f"Model: {model_name}", (30, 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2)
        cv2.putText(frame_copy, f"Mulai dalam {sec} detik...", (30, 150),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
        cv2.imshow("YOLOv11 OBB Monitoring", frame_copy)
        cv2.waitKey(1000)

    print(f"🚀 Mulai evaluasi model: {model_name}")
    ...
    while frame_count < 1000:
        ...
        # Overlay info
        cv2.putText(annotated_frame, f"Model: {model_name}", (10, 15),
